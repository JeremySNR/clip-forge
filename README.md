...
1. **Import** any local video (MP4, MOV, MKV, WEBM…).
...
5. **Edit** each clip: trim with draggable handles, reframe to 9:16 / 1:1 / 16:9 (fill-crop with a focus slider, or fit with a blurred background), pick a caption style, toggle the AI hook title.
6. **Preview live** — playback is bounded to the clip, reframing and word-by-word captions are simulated in real time before you render anything.
7. **Export** MP4s with captions burned in (libx264 + AAC, faststart), one clip at a time or all at once.
...
│   │   └── render.ts      cut, reframe, caption burn-in, encode
...
./scripts/smoke-test.sh                                          # UI screenshot under Xvfb
npm run typecheck
```
...
- Active-speaker detection for automatic reframing
