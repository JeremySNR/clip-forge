...
import { reviewPanelLabels } from '../src/main/pipeline/presenterComposition'
...
it('renders all three proof labels with spaces, apostrophes and graph delimiters in the font path', async () => {
  ...
  try {
    ...
    await runFfmpeg(['-f', 'lavfi', '-i', 'color=c=black:s=1360x32:r=1:d=1',
      '-vf', reviewPanelLabels(font), '-frames:v', '1', '-pix_fmt', 'gray', '-f', 'rawvideo', output])
    const pixels = await readFile(output)
    expect(pixels.length).toBe(1360 * 32)
