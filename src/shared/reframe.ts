...
function needsLayoutUpgrade(clip: Clip): boolean {
  // Only completed analyses: a pending editorial letterbox (or one the user
  // picked while framing was outstanding) must not look like a v0.11 upgrade.
  return Boolean(clip.reframeStatus !== 'pending' &&
    clip.visualLayout?.preserveContext && !clip.visualLayout.revision &&
    !clip.visualLayout.shots?.some(s => s.composition || s.region || s.mode === 'crop') &&
    (clip.reframeAnalysis?.version ?? 0) < 2 &&
